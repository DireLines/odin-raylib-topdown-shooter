...
_read_entire_file :: proc(
	...
	if file == nil {
		print("Failed to open file %v", name)
		return
	}
	...
	if size <= 0 {
		print("Failed to read file %v", name)
		return
	}
	...
	if data_err != nil {
		print("Error allocating memory: %v", data_err)
		return
	}
	...
	if read_size != c.size_t(size) {
		print("File %v partially loaded (%i bytes out of %i)", name, read_size, size)
	}

	print("Successfully loaded %v", name)
	return data, true
}
...
_write_entire_file :: proc(name: string, data: []byte, truncate := true) -> (success: bool) {
	...
	if file == nil {
		print("Failed to open '%v' for writing", name)
		return
	}
	...
	if bytes_written == 0 {
		print("Failed to write file %v", name)
		return
	} else if bytes_written != len(data) {
		print("File partially written, wrote %v out of %v bytes", bytes_written, len(data))
		return
	}

	print("File written successfully: %v", name)
	return true
}
