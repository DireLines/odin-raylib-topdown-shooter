...
//game which just shows a black screen
...
game_update :: proc(dt: f64) {
	{
		it := object_iter()
		for obj in all_objects_with_tags(&it, .Squish) {
			obj.display_transform = {}
			squish := f64(game.frame_counter) / 20
			obj.display_transform.scale = vec2{1, 1} + 0.01 * {math.sin(squish), -math.sin(squish)}
		}
	}
	{
		it := object_iter()
