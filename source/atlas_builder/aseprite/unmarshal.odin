...
unmarshal_from_bytes_buff :: proc(
	doc: ^Document,
	r: ^bytes.Reader,
...
unmarshal_from_file :: proc(
	...
	rr, ok := io.to_reader(os.to_stream(f))
	if !ok {
		return .Unable_Make_Reader
	}
	return unmarshal(doc, rr, alloc)
...
unmarshal :: proc {
	unmarshal_from_bytes_buff,
	unmarshal_from_slice,
	unmarshal_from_file,
	...
	for &frame in doc.frames {
		...
		for &chunk in frame.chunks {
			...
			case .old_palette_256:
				old_palette := read_old_palette_256(r, rt) or_return
				chunk = old_to_new_palette_256(old_palette)

			case .old_palette_64:
				old_palette := read_old_palette_64(r, rt) or_return
				chunk = old_to_new_palette_64(old_palette)
...
unmarshal_chunks :: proc {
	unmarshal_multi_chunks,
	unmarshal_single_chunk,
}


unmarshal_multi_chunks :: proc(
	r: io.Reader,
	buf: ^[dynamic]Chunk,
	chunks: Chunk_Set,
	alloc := context.allocator,
...
old_to_new_palette_256 :: proc(old: Old_Palette_256_Chunk) -> (new: Palette_Chunk) {
	assert(len(old) == 1)
	packet := old[0]
	new.entries = make([]Palette_Entry, packet.num_colors)
	for i in packet.entries_to_skip ..< packet.entries_to_skip + packet.num_colors {
		color := packet.colors[i]
		new_ind := i - packet.entries_to_skip
		new.entries[i] = Palette_Entry {
			color = Color_RGBA{color.r, color.g, color.b, 255},
		}
		new.size += 1
	}
	return new
...
old_to_new_palette_64 :: proc(old: Old_Palette_64_Chunk) -> (new: Palette_Chunk) {
	assert(len(old) == 1)
	packet := old[0]
	new.entries = make([]Palette_Entry, packet.num_colors)
	for i in packet.entries_to_skip ..< packet.entries_to_skip + packet.num_colors {
