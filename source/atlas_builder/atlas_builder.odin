...
main :: proc() {
	...
	img_write :: proc "c" (ctx: rawptr, data: rawptr, size: c.int) {
		...
		_ = os.write_entire_file(ATLAS_PNG_OUTPUT_PATH, slice.bytes_from_ptr(data, int(size)))
	}
	...
	f, _ := os.open(ATLAS_ODIN_OUTPUT_PATH, {.Write, .Create, .Trunc}, os.perm_number(0x1A4))
	defer os.close(f)

	fmt.fprintfln(
		f,
		"// This file was generated by running the atlas_builder with odin version %s (%s).",
		ODIN_VERSION,
		ODIN_VERSION_HASH,
	)
	...
	fmt.fprintln(f, "\t.None = {document_size ={0,0}},")
	...
	fmt.fprintln(f, "TagLoopDir :: enum {")
	...
	fmt.fprintln(f, "\tdocument_size: [2]f32,")
	fmt.fprintln(f, "\tloop_direction: TagLoopDir,")
	...
	fmt.fprintln(f, "atlas_fonts: [FontName][]AtlasGlyph = {")
	for font in atlas_fonts {
		fmt.fprintf(f, "\t.%v = {{\n", font.name)
