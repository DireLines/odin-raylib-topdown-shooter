...
HandleMap :: struct($T: typeid, $HT: typeid, $N: int) {
...
add :: proc(m: ^HandleMap($T, $HT, $N), v: T) -> (HT, bool) #optional_ok {
	v := v
...
get :: proc(m: ^HandleMap($T, $HT, $N), h: HT) -> (^T, bool) #optional_ok {
...
remove :: proc(m: ^HandleMap($T, $HT, $N), h: HT) {
	if h.idx <= 0 || h.idx >= m.num_items {
		return
	}
...
//noop to comply with other handle maps which do have a delete
delete :: proc(m: ^HandleMap($T, $HT, $Max), loc := #caller_location) {
}

// For iterating a handle map. Create using `make_iter`.
HandleMap_Iterator :: struct($T: typeid, $HT: typeid, $N: int) {
	m:     ^HandleMap(T, HT, N),
	index: u32,
}

// Create an iterator. Use with `iter` to do the actual iteration.
make_iter :: proc(m: ^HandleMap($T, $HT, $N)) -> HandleMap_Iterator(T, HT, N) {
	return {m = m, index = 1}
}
...
iter :: proc(it: ^HandleMap_Iterator($T, $HT, $N)) -> (val: ^T, h: HT, cond: bool) {
